...
from acestep.ui.gradio.events.library_ratings import load_ratings, save_ratings
...
def delete_song(audio_path: str) -> tuple:
    ...
    try:
        ...
        for sidecar in [".json", ".repaint_latents.npy", ".session.npz", ".vtt"]:
            sidecar_path = stem + sidecar
            if os.path.exists(sidecar_path):
                os.remove(sidecar_path)
        # Remove the parent folder if it is now empty and looks like a batch dir
        parent = os.path.dirname(real)
        if (parent and os.path.isdir(parent) and not os.listdir(parent)
                and os.path.basename(parent).startswith("batch_")):
            os.rmdir(parent)
        ratings = load_ratings()
        ratings.pop(audio_path, None)
        save_ratings(ratings)
